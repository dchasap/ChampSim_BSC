																			...
																			CACHE::REP_POL_XARGS xargs)
{
	if (xargs.is_instr) {
		...
		*/
		// cache hit
		if (hit) {
			::least_recently_used[this][set * NUM_WAY + way] = maxRRPV - data_pos; // for cache hit, DRRIP always promotes
	...
	if (acc_freq < 50) {
		::least_recently_used[this][set * NUM_WAY + way] = maxRRPV - instr_pos; //instr and data pos are in reverse here
	}
	else {
