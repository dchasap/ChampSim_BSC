...
{
	if (getenv("ITP_MIN_EVICTION_POSITION")) {
		::MIN_EVICTION_POSITION[this] = std::stoi(getenv("ITP_MIN_EVICTION_POSITION"));
	}

	std::cout << NAME << " is using ITP" << std::endl; 
...
*/
		if (::least_recently_used[this][set * NUM_WAY + i].is_data
...
//	std::cout << "alt_lru:" << max_alt_lru << std::endl;
	if (max_alt_lru >= ::MIN_EVICTION_POSITION[this]) {
