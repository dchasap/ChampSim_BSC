...
class FDIP {
  ...
  public:
    FDIP(uint64_t agg) : width(6), offset(10*width), aggressivity(static_cast<uint32_t>(agg)), prune_factor(0), enabled (agg > 0) {
    ...
    void push_back(std::deque<ooo_model_instr>::iterator instr) {
        ...
        champsim::address ip = instr->ip;
        if (std::find_if(   std::begin(BUFFER), 
                            std::end(BUFFER), 
                            [ip] (auto x) { 
                                return ((ip.to<uint64_t>() >> LOG2_BLOCK_SIZE) == (x.ip.template to<uint64_t>() >> LOG2_BLOCK_SIZE));
                            }) == std::end(BUFFER)){
            BUFFER.push_back(*instr);
        }
    }

    std::pair<champsim::address, uint64_t> get_prefetch_line() {
        assert(!empty());
        champsim::address ip = BUFFER.front().ip;
